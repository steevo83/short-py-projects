...
def meetup_date(year, month, nth=4, weekday=3):
    third_w = datetime.date(year, month, 22).weekday()
    return datetime.date(year, month, 22+(3-third_w) % 7)
...
# The arguments, "nth" and "weekday", will allow callers of your function to specify which weekday their meetup is held on and which occurrence in the month it is held 
# (for example the 2nd Tuesday or the first Friday).
